---
name: rakka-accel-backends
description: Use when choosing between portable (`rakka-accel` core trait surface) and vendor-specific (`rakka-accel-cuda`) APIs. Covers when to write generic code over `AccelBackend`, what's in `AccelRef<T, B>` / `AccelError` / `CompletionStrategy`, and the multi-vendor roadmap (ROCm, Metal, oneAPI, Vulkan compute). Triggers on "should I depend on `rakka-accel` or `rakka-accel-cuda`?", "is this code portable?", or design questions about the trait surface.
---
...
| Goal | Depend on |
|---|---|
| Drive an NVIDIA GPU; you don't expect to support other vendors | `rakka-accel-cuda` directly |
| Write portable algorithms that should run on any backend rakka-accel ships | `rakka-accel` (with `cuda` feature for the active backend) |
| Both — public API portable, internals CUDA-specific | depend on both; expose the portable surface in your public API |

Most projects today should depend on `rakka-accel-cuda` directly
...
The core crate (`rakka-accel`) defines:
...
The CUDA crate (`rakka-accel-cuda`) **adds**:
...
The blueprint sub-crates (`rakka-accel-patterns`,
`rakka-accel-train`, `rakka-accel-agents`) are
...
When `rakka-accel-rocm` lands, `observe::<RocmBackend>` works
without changes.

### 2. Library code reused by both backend-generic and CUDA-specific consumers

Define the public surface in terms of the `rakka-accel` trait:

```rust
// In your library:
use rakka_accel::AccelError;
...
backend, just depend on `rakka-accel-cuda`. The portable layer
...
|---|---|---|
| **CUDA (NVIDIA)** | **Shipping** in `rakka-accel-cuda` | Full F2-F9 + rakka 0.2 integration. |
...
| WebGPU | Not planned | Targets browsers / edge devices; doesn't share rakka's actor / supervision concerns. |
...
6. (Optional) feature-flag in `rakka-accel/Cargo.toml`
   re-exporting at `rakka_accel::<name>`.

See `crates/rakka-accel-cuda/` as the reference implementation.
...
- `crates/rakka-accel/src/lib.rs` — what the core crate exports.
- `crates/rakka-accel/src/backend.rs` — `AccelBackend`,
  `AccelDevice`, `AccelStream` traits.
- `crates/rakka-accel/src/error.rs` — `AccelError` (the typed
  enum every backend wraps or re-exports).
- `crates/rakka-accel-cuda/src/lib.rs` — reference impl entry
  point.
...
- **Pulling `rakka-accel` without enabling `cuda`.** The umbrella
  ships only the trait surface by default. If you want the
  concrete actors, depend on `rakka-accel-cuda` directly or
