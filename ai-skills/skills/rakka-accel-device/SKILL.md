...
is a no-op if you didn't enable `rakka-accel-cuda/cudnn`.
...
- `crates/rakka-accel-cuda/src/device/device_actor.rs` —
  `DeviceMsg` definition and dispatch.
- `crates/rakka-accel-cuda/examples/sgemm.rs` — full SGEMM smoke
  test.
- [`rakka-accel-kernels`](../rakka-accel-kernels/SKILL.md) — for
  per-library kernels.
- [`rakka-accel-supervision`](../rakka-accel-supervision/SKILL.md)
