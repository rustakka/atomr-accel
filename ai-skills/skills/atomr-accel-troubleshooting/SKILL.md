---
name: atomr-accel-troubleshooting
description: Use when diagnosing failures in a project that depends on atomr-accel. Covers compile-time errors (missing feature flags, wrong prelude), runtime issues (`GpuRefStale`, OOM loops, mailbox stalls, no-GPU host vs CI gating), Python wheel installation problems, and CUDA-driver-not-loaded misdiagnoses. Triggers on atomr-accel error messages, "missing symbol" errors, hangs, repeated restarts, or any "why isn't my GPU work running" question.
---

# Troubleshooting atomr-accel
...
[`atomr-accel-python`](../atomr-accel-python/SKILL.md)).

## Compile-time

### "cannot find type `X` in crate `atomr_accel_cuda`"
...
use atomr::prelude::*;     // Actor, ActorRef, Context, Props
```
...
```toml
atomr-accel-cuda = { features = ["full-cuda"] }   # ✓
atomr-accel      = { features = ["cuda"] }        # ✓ (umbrella)
```

Don't mix them: feature names are **per crate**, so
`atomr-accel = { features = ["cudnn"] }` won't work — `cudnn`
is on `atomr-accel-cuda`.
...
binary. If `nvidia-smi` succeeds but atomr-accel fails, dlopen
...
### `ImportError: cannot import name 'RngGenerator' from 'atomr_accel'`
...
if atomr_accel.RngGenerator is not None:
    ...
    raise RuntimeError("install atomr-accel with [curand] feature")
```

### `pip install atomr-accel` fails with "no matching distribution"
...
- `crates/atomr-accel-cuda/src/error.rs` — `GpuError` variants
  and their meanings.
- `crates/atomr-accel-cuda/tests/end_to_end_e2e.rs` — minimal
