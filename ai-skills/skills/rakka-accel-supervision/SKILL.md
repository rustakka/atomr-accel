...
# Supervision and recovery in rakka-accel
...
[`rakka-accel-troubleshooting`](../rakka-accel-troubleshooting/SKILL.md).
...
rakka-accel mirrors this with two actors — a stable outer one
...
up the tree. rakka-accel encodes the directive in the panic
message:
...
that, [`rakka-accel-cuda::error::DeviceSupervisor`](../../../crates/rakka-accel-cuda/src/error.rs)
implements rakka 0.2's typed `SupervisorOf<ContextActor>` trait
over `GpuError`, so you can pattern-match on the typed enum
directly:

```rust
use rakka_accel_cuda::error::DeviceSupervisor;
...
- You're embedding rakka-accel inside a larger actor system and
...
- `crates/rakka-accel-cuda/src/error.rs` — `GpuError`,
  `decider()`, `DeviceSupervisor`, the panic-tag constants.
- `crates/rakka-accel-cuda/src/device/state.rs` — `DeviceState`
  with the generation counter and watch channel.
- `crates/rakka-accel-cuda/tests/supervisor_decider.rs` — round
...
- **Assuming `pre_start` re-runs on restart.** rakka does **not**
