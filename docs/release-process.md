...
`atomr-accel` release. It covers the day-to-day workflow ("I want to
...
The release flow mirrors the sibling [atomr](https://github.com/rustakka/atomr)
workspace's process so the two projects stay in lockstep on tooling.
...
* End-to-end takes ~20 minutes from `git push` to "all 10 crates on
  crates.io and the wheel set on PyPI."
* If you need to ship something that isn't a real fix/feat, append a
  `Release-As: x.y.z` footer to any commit body to force an
  exact-version bump.
...
This is the same bug the upstream atomr workspace hit between v0.6.1
and v0.9.1: five tags landed in git and zero of them ever ran
`release.yml`, so crates.io and PyPI stayed frozen for four versions.
atomr-accel adopted the trampoline from day one to avoid the same
trap.
...
* **Use a GitHub App token** — also works, but requires installing
...
|---|---|---|
| 10 Rust crates (workspace publishables) | crates.io | `publish-crates` job, sequentially in dep order |
...
Wheels are abi3-py310 (one wheel per platform covers Python 3.10–3.13
via the stable ABI).

Unlike the upstream atomr workspace, atomr-accel does **not** ship
pre-built CLI binaries — there are no equivalents to `atomr-dashboard`
or `atomr-profiler`. If a binary tool ever ships, slot in a
`build-binaries` matrix mirroring atomr's release.yml.
...
─────  ──────────────────────────────────────────────────────────────
  ...
  2    atomr-accel-telemetry, atomr-accel-flashattn,
       atomr-accel-tensorrt, atomr-accel-cutlass
  3    atomr-accel-cuda  (depends on atomr-accel + the layer-2 leaves)
  4    atomr-accel-patterns, atomr-accel-agents,
       atomr-accel-cuda-realtime  (each depends only on
                                   atomr-accel-cuda)
...
Workspace members deliberately excluded from the publish list:

* `xtask` — `publish = false` in its `Cargo.toml`.
* `crates/atomr-accel-py` — `publish = false`; it ships as the single
  `atomr-accel` PyPI wheel via maturin, not as a separate crates.io
  package.
* `crates/atomr-accel-cub` — `publish = false` until Phase 5.1 lands
  the NVRTC kernel emitters. Today every dispatch returns
  `GpuError::Unrecoverable("kernel compile path lands in Phase 5.1")`,
  so a crates.io publish would ship an unusable stub. When Phase 5.1
  lands, remove the `publish = false` flag and slot the crate into
  layer 4 of `publish-crates` (it depends on `atomr-accel-cuda`).
...
4. If your crate is purely internal (xtask, py-bindings, examples),
   add `publish = false` to its `[package]` table so `cargo publish`
   rejects accidental publishes.

## PyPI publishing

`atomr-accel` ships as a single PyPI distribution (the project name is
`atomr-accel`), built by [`maturin`](https://www.maturin.rs/) from the
`crates/atomr-accel-py` crate. The Python module is `atomr_accel` and
the native extension is `atomr_accel._native`.
...
Every release-relevant workflow checks out two repos side-by-side:
...
This is required because `[workspace.dependencies]` in
`atomr-accel/Cargo.toml` carries `path = "../atomr/crates/..."` pins
on `atomr-core`, `atomr-config`, `atomr-macros`, `atomr-persistence`,
`atomr-cluster-sharding`, `atomr-streams`, `atomr-telemetry`, and
`atomr-testkit`. The `version = "X.Y.Z"` literal next to each path
dep is what gets published to crates.io, but the path dep is what
local builds and CI use.

When bumping the atomr workspace pin, update both the path-dep
versions in `[workspace.dependencies]` and the toolchain assumption
in any cross-repo CI job. The `cargo xtask bump` command rewrites
the workspace pins in place; a manual edit of the
`version = "..."` literals on the path-dep lines is also required when
following a sibling atomr release.
...
2. Add it as a repo secret named `CRATES_IO_TOKEN` scoped to the
   `crates-io` GitHub environment (Settings → Environments →
   `crates-io` → Secrets).
...
1. Create the project on the relevant PyPI host (or upload one wheel
   manually first). atomr-accel's TestPyPI project is already
   provisioned.
...
Runs the verify gate, builds every wheel + sdist, runs `cargo publish
--dry-run` on every crate, and uploads to TestPyPI. Skips crates.io
and production PyPI. Use this to rehearse a release after non-trivial
...
* Following the upstream atomr release version (atomr-accel mirrors
  the atomr workspace version)
...
### "tags exist but nothing published"
...
and all 10 crates publish at the new version, jumping over the
...
### "failed to select a version for the requirement `atomr-accel-foo = ^X.Y.Z`"
...
### "failed to select a version for the requirement `atomr-core = ^X.Y.Z`"

The sibling-atomr pin in `[workspace.dependencies]` references a
version of an upstream `atomr-*` crate that isn't on crates.io yet.
The atomr workspace publishes ahead of atomr-accel; if you've bumped
atomr-accel before the matching atomr release lands, the publish job
will block.

Fix: wait for the matching atomr release to land on crates.io, then
re-tag the same atomr-accel version. The `already uploaded` skip
...
* You add a new artifact target (new wheel platform, future binary).
