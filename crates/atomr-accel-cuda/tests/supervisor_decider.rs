...
//! that the atomr actor cell honours that directive end-to-end.
...
async fn context_poisoned_panic_triggers_restart() {
    ...
    // post_restart hooks (which do not call pre_start in atomr — but the
    ...
    // logic only panics on `n == 1` — but since atomr doesn't increment
