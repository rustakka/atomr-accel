...
use atomr_accel_cuda::kernel::{CsrMatrix, SparseActor, SparseMsg};
...
async fn alloc_and_copy_f32(
    ...
    dev.tell(DeviceMsg::CopyFromHostF32 {
        src: atomr_accel_cuda::device::HostBuf::Vec(host.to_vec()),
        dst: g.clone(),
        reply: tx,
    });
...
async fn alloc_and_copy_i32(
    dev: &atomr_core::actor::ActorRef<DeviceMsg>,
    host: &[i32],
) -> atomr_accel_cuda::gpu_ref::GpuRef<i32> {
    let (tx, rx) = oneshot::channel();
...
async fn copy_to_host_f32(
    ...
    dev.tell(DeviceMsg::CopyToHostF32 {
        src: g.clone(),
        dst: atomr_accel_cuda::device::HostBuf::Vec(vec![0.0; len]),
        reply: tx,
    });
    match rx.await.unwrap().unwrap() {
        atomr_accel_cuda::device::HostBuf::Vec(v) => v,
        atomr_accel_cuda::device::HostBuf::Pinned(_) => panic!("expected Vec"),
    }
}
