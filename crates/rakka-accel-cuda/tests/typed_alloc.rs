...
use rakka_testkit::TestKit;

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn mock_allocate_typed_variants() {
    let kit = TestKit::new("alloc-test").await; let sys = &kit.system;
    let dev = sys
...
async fn legacy_allocate_alias_still_works() {
    let kit = TestKit::new("alloc-legacy").await; let sys = &kit.system;
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
...
async fn stats_returns_load_snapshot() {
    let kit = TestKit::new("stats-test").await; let sys = &kit.system;
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
...
async fn snapshot_context_returns_none_in_mock_mode() {
    let kit = TestKit::new("snapshot-test").await; let sys = &kit.system;
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
