...
//!
//! Rakka's supervisor (`rakka_core::supervision::Decider`) inspects a panic
...
use rakka_core::supervision::{Directive, OneForOneStrategy, SupervisorStrategy};
...
impl GpuError {
    ...
    /// rakka supervisor's decider can route it.
...
/// failures (§5.11).
///
/// Deviation from the architecture doc: the doc shows
/// `OneForOne::default().on::<ContextPoisoned>(Directive::Restart)`. Rakka
/// has no typed-error supervision API, so we string-match panic messages
/// instead. Acceptable for F1; F2 should propose a typed-error
/// `Decider` upstream.
