...
pub mod sparse {
    //! cuSPARSE wrapper actor — placeholder.
    //!
    //! cudarc 0.19 only exposes cuSPARSE at the `sys` layer; no
    //! safe wrapper. This module compiles (so the feature flag
    //! is meaningful) but does not yet expose an actor; track
    //! cudarc upstream for a safe wrapper before we wire one.
    pub const STATUS: &str = "F5 placeholder; needs cudarc cuSPARSE safe wrapper";
}

#[cfg(feature = "cutensor")]
pub mod tensor {
    //! cuTENSOR wrapper actor — placeholder.
    //!
    //! cudarc 0.19's cuTENSOR safe layer is minimal. Track
    //! upstream for tensor-contraction primitives before
    //! wrapping as an actor.
    pub const STATUS: &str = "F5 placeholder; needs cudarc cuTENSOR safe surface";
}
