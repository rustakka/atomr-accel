...
async fn mock_allocate_typed_variants() {
    let sys = ActorSystem::create("alloc-test", Config::empty()).await.unwrap();
    let dev = sys
    ...
    // U8
    let (tx, rx) = oneshot::channel();
    dev.tell(DeviceMsg::AllocateU8 { len: 1024, reply: tx });
    let r = tokio::time::timeout(Duration::from_secs(2), rx).await.unwrap().unwrap();
    assert!(matches!(r, Err(GpuError::Unrecoverable(_))));
...
async fn legacy_allocate_alias_still_works() {
    let sys = ActorSystem::create("alloc-legacy", Config::empty()).await.unwrap();
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
...
async fn stats_returns_load_snapshot() {
    let sys = ActorSystem::create("stats-test", Config::empty()).await.unwrap();
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
    ...
    let load = tokio::time::timeout(Duration::from_secs(2), rx).await.unwrap().unwrap();
...
async fn snapshot_context_returns_none_in_mock_mode() {
    let sys = ActorSystem::create("snapshot-test", Config::empty()).await.unwrap();
    let dev = sys
        .actor_of(DeviceActor::props(DeviceConfig::mock(0)), "dev0")
        .unwrap();
