...
//! [`rakka_accel_cuda::kernel::NvrtcActor`] and dispatches `SpMv` through
//! the JIT-launched kernel. The cuSPARSE-backed variant lives in
//! `rakka-accel-cuda` proper (Phase C.1) and is the preferred backend when
//! that feature is on.
    ...
    async fn handle(&mut self, _ctx: &mut Context<Self>, msg: SparseMsg) {
        match msg {
            ...
            SparseMsg::SpMv { x, reply } => {
                ...
                // `rakka_accel_cuda::pipeline` — added in a follow-up that
