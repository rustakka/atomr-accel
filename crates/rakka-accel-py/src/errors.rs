...
create_exception!(rakka_accel, GpuRuntimeError, PyException, "Base rakka-accel error.");
create_exception!(rakka_accel, ContextPoisoned, GpuRuntimeError, "CUDA context poisoned; supervisor will restart.");
create_exception!(rakka_accel, OutOfMemory, GpuRuntimeError, "Device-side OOM; supervisor resumes the actor.");
create_exception!(rakka_accel, Unrecoverable, GpuRuntimeError, "Hardware fault or past retry budget; device stops.");
create_exception!(rakka_accel, GpuRefStale, GpuRuntimeError, "GpuRef used after context rebuild.");
create_exception!(rakka_accel, LibraryError, GpuRuntimeError, "Library-level CUDA error (cuBLAS/cuDNN/...).");
create_exception!(rakka_accel, AskTimeout, GpuRuntimeError, "Ask exceeded its timeout budget.");

pub fn register(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add("GpuRuntimeError", m.py().get_type_bound::<GpuRuntimeError>())?;
    m.add("ContextPoisoned", m.py().get_type_bound::<ContextPoisoned>())?;
...
pub fn map_gpu(e: GpuError) -> PyErr {
    ...
    match e {
        ...
        GpuError::LibraryError { .. } | GpuError::Driver(_) => {
            PyErr::new::<LibraryError, _>(msg)
        }
